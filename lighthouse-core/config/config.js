...
class Config {
   ...
   * relative to the optional `configDir`) using `Runner.resolveModule`,
    ...
    const auditDefns = expandedAudits.map(audit => {
      ...
      if ('implementation' in audit) {
        ...
        if (!coreAudit) {
          ...
          requirePath = Runner.resolveModule(audit.path, configDir, 'audit');
        }
  ...
  static requireGathererFromPath(path, options, coreAuditList, configDir) {
    ...
    if (!coreGatherer) {
      // Otherwise, attempt to find it elsewhere. This throws if not found.
      requirePath = Runner.resolveModule(path, configDir, 'gatherer');
    }
   ...
   * provided) using `Runner.resolveModule`, returning an array of full Passes.
   * @param {?Array<Required<LH.Config.PassJson>>} passes
   * @param {string=} configDir
   * @return {Config['passes']}
   */
